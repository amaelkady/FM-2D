...
if ModelELOption==2
    ...
    for Floor=NStory+1:-1:2
        ...
        for Axis=1:NBay+1
            ...
            H_BEAM =  WBay(Bay) - 0.5*SecData.d(idxCi) - 0.5*SecData.d(idxCj) - 2*L_RBS;
            ...
            if Axis~=1 && Axis~=NBay+1
                fprintf(INP,'Spring_IMK %d %d %d $E $fy [expr $Comp_I*%.3f] %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f 0.0 $Composite %d %d; ',SpringID_L,Node20,Node02,I_RBS, SecData.d(idx),SecData.h_tw(idx),SecData.bf_tf(idx), SecData.ry(idx), H_BEAM, L_BEAM, Lb_BEAM, My, MFconnection, Units);
                fprintf(INP,'Spring_IMK %d %d %d $E $fy [expr $Comp_I*%.3f] %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f 0.0 $Composite %d %d; ',SpringID_R,Node04,Node40,I_RBS, SecData.d(idx),SecData.h_tw(idx),SecData.bf_tf(idx), SecData.ry(idx), H_BEAM, L_BEAM, Lb_BEAM, My, MFconnection, Units);
            end
            if Axis==1
                fprintf(INP,'Spring_IMK %d %d %d $E $fy [expr $Comp_I*%.3f] %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f 0.0 $Composite %d %d; ',SpringID_R,Node04,Node40,I_RBS, SecData.d(idx),SecData.h_tw(idx),SecData.bf_tf(idx), SecData.ry(idx), H_BEAM, L_BEAM, Lb_BEAM, My, MFconnection, Units);
            end
