function plot_PH_state (app, Ti, dispfloor)
...
for Fi = 1:NStory+1
    ...
    for Axis = 1:NBay+1+xx
        ...
        elseif  Fi==NStory+1
            ...
            if Ti < indxP
                plot(app.axes1,GridX(Axis)+dispfloor(Fi), GridY(Fi)-Beam_Data.d(Fi,1),'o','Markeredgecolor',[0.4660 0.6740 0.1880],'Markerfacecolor','g','MarkerSize',5);
            elseif Ti >= indxP &&  Ti < indxC
                plot(app.axes1,GridX(Axis)+dispfloor(Fi), GridY(Fi)-Beam_Data.d(Fi,1),'o','Markeredgecolor',[0.8500 0.3250 0.0980],'Markerfacecolor',[0.9290 0.6940 0.1250],'MarkerSize',5);
            elseif Ti > indxC
                plot(app.axes1,GridX(Axis)+dispfloor(Fi), GridY(Fi)-Beam_Data.d(Fi,1),'o','Markeredgecolor',[0.6350 0.0780 0.1840],'Markerfacecolor','r','MarkerSize',5);
            end
        else
            Sub_RootName = 'T';
            evalc(['F_Data=','''',RootName,num2str(Fi),num2str(Axis),Sub_RootName,'_F','''']);
