...
try
    for i=1:NStory+1
    	evalc(strcat('sizeRFA(',num2str(i),')=size(DATA.RFA',num2str(i),',1)'));
    end
    ...
    for i=1:NStory+1
        ...
        if sizeRFA(i) == size(DATA.EQ_Inter,1)+11
        	...
        	evalc(strcat('DATA.PFA',num2str(i),'= DATA.RFA', num2str(i), '(12:end,1)/g + SFcurrent * DATA.EQ_Inter(1:sizeRFA(i),1)-11','1)'));
        end
    end
catch ME
    ME
