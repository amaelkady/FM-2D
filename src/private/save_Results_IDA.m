
function save_Results_IDA(MainDirectory,RFpath,GM_No,GM,NStory,IncrNo,IncrDATA,RECORDERS)

%% Delete All OpenSEES Output Files
if RECORDERS.Delete_Flag==1
    cd (strcat(RFpath,'\Results\',GM.name{:,GM_No}));
    delete *.out
    cd (MainDirectory);
end

%% Save Predetermined Limit Values at End of Vectors for IDA Ploting Reasons
IncrDATA.SA         (IncrNo) = max(IncrDATA.SA);

if RECORDERS.SDR==1
    IncrDATA.SDR_Max    (IncrNo) = 0.2;
    IncrDATA.RDR_Max    (IncrNo) = 0.2;
    for i=1:NStory
        IncrDATA.SDR(i,IncrNo)=0.2;
        IncrDATA.RDR(i,IncrNo)=0.2;
    end
end

if RECORDERS.RFA==1
    for i=1:NStory+1
        IncrDATA.PFA(i,IncrNo)= max (IncrDATA.PFA(i,:));
    end
end

%% Save Processed IDA Data to a Text File in a Different Folder for Each GM
cd (strcat(RFpath,'\Results\',GM.name{:,GM_No}));

if RECORDERS.SDR==1
    % SDR IDA DATA
    file5 = fopen('IDA SDR.txt','wt');
    for incr=1:IncrNo
        fprintf(file5,'%f\t',IncrDATA.SA(incr));
        for i = 1:NStory
            fprintf(file5,'%f\t',IncrDATA.SDR(i,incr));
        end
        fprintf(file5,'\n');
    end
    fclose(file5);

    % RDR IDA DATA
    file7 = fopen('IDA RDR.txt','wt');
    for incr=1:IncrNo
        fprintf(file7,'%f\t',IncrDATA.SA(incr));
        for i = 1:NStory
            fprintf(file7,'%f\t',IncrDATA.RDR(i,incr));
        end
        fprintf(file7,'\n');
    end
    fclose(file7);
end

if RECORDERS.RFA==1
    % PFA IDA DATA
    file6 = fopen('IDA PFA.txt','wt');
    for incr=1:IncrNo
        fprintf(file6,'%f\t',IncrDATA.SA(incr));
        for i = 1:NStory+1
            fprintf(file6,'%f\t',IncrDATA.PFA(i,incr));
        end
        fprintf(file6,'\n');
    end
    fclose(file6);
end

cd (MainDirectory);