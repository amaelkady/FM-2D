...
if Modelstatus==1
    ...
    SummaryText{i}=['- Tcl File Name       : ',OpenSEESFileName];i=i+1;
    ...
    SummaryText{i}='------------------------------------------------------';i=i+1;
    if ModelELOption==1
        SummaryText{i}=['- Model Type                     : Nonlinear'];i=i+1;
    else
        SummaryText{i}=['- Model Type                     : Linear Elastic'];i=i+1;
    end
    ...
    if CompositeX==1
        SummaryText{i}=['- Composite floor action         : Considered'];i=i+1;
    else
        SummaryText{i}=['- Composite floor action         : Not Considered'];i=i+1;
    end
    if GFX==1
        if Orientation==1; SummaryText{i}=['- Gravity framing system         : Considered with weak-axis orientation with respect to the modeled direction.'];i=i+1; end
        if Orientation==2; SummaryText{i}=['- Gravity framing system         : Considered with strong-axis orientation with respect to the modeled direction.'];i=i+1; end
    else
        SummaryText{i}=['- Gravity framing system         : Not Considered'];i=i+1;
    end
    if PZ_Multiplier==1
        SummaryText{i}=['- Panel zone deformation         : Considered'];i=i+1;
    else
        SummaryText{i}=['- Panel zone deformation         : Not Considered'];i=i+1;
    end
    if RigidFloor==1
        SummaryText{i}=['- Rigid floor diaphragm movement : Considered'];i=i+1;
    else
        SummaryText{i}=['- Rigid floor diaphragm movement : Not Considered'];i=i+1;
    end
end
...
if Definestatus==1
        ...
        if FrameType==4;                     SummaryText{i}=['- Frame Type                       : RC MRF'];i=i+1;        end
        ...
        SummaryText{i}=['- Poisson ratio                    : ', num2str(mu0),matUnit];i=i+1;
        SummaryText{i}='';i=i+1;
    
    SummaryText{i}='SUPPORTS & CONNECTIONS';i=i+1;
    SummaryText{i}='------------------------------------------------------';i=i+1;
    if Support==1
        SummaryText{i}=['- Main frame column supports  : Fixed'];i=i+1;
    else
        SummaryText{i}=['- Main frame column supports  : Pinned'];i=i+1;
    end
    if SpliceConnection==1
        SummaryText{i}=['- Column splices              : Pinned'];i=i+1;
    else
        SummaryText{i}=['- Column splices              : Fixed'];i=i+1;
    end
    if FrameType==1
        if MFconnection==0
            SummaryText{i}=['- Beam-to-column connections: RBS connection'];i=i+1;
    ...
    if ColElementOption==1
        SummaryText{i}=['- Column members model     : Lumped plasticity approach'];i=i+1;
    elseif ColElementOption==2
        SummaryText{i}=['- Column members model     : Displacement-based fiber elements'];i=i+1;
    elseif ColElementOption==3
        SummaryText{i}=['- Column members model     : Force-based fiber elements'];i=i+1;
    end
    SummaryText{i}='------------------------------------------------------';i=i+1;
end
...
if Analysisstatus==1
    ...
    elseif ELF==1
        SummaryText{i}=['- Analysis Type                    : ELF'];i=i+1;
    elseif IDA==1
