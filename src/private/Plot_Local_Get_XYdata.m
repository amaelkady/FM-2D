...
global MainDirectory NBay Units
...
if strcmp(Element,'Column Spring')==1
    ...
    X=D_Data(:,3)*100;
    Y=F_Data(:,DataColIndx)*convfactor;
...
elseif strcmp(Element,'EGF Connection')==1
    evalc(['F_Data=importdata(','''',yRootName,num2str(Floor+1),num2str(NBay+1+Axis),Sub_RootName,'_F.out','''',')']);
    evalc(['D_Data=importdata(','''',xRootName,num2str(Floor+1),num2str(NBay+1+Axis),Sub_RootName,'_D.out','''',')']);
    X=D_Data(:,3)*100;
    Y=F_Data(:,DataColIndx)*convfactor;
