...
for Story=1:NStory
    if Splice(Story,1)
        Fi=Story;
        for Axis=1:NBay+3
            if SpliceConnection==1; plot(app.axes1,[GridX(Axis)],[GridY(Fi)+HStory(Story,1)*Splice(Story,2)],'ok','MarkerSize',3); end
            if SpliceConnection==2; plot(app.axes1,[GridX(Axis)],[GridY(Fi)+HStory(Story,1)*Splice(Story,2)],'k','Marker','square','MarkerSize',3); end
        end
    end
end
drawnow;
