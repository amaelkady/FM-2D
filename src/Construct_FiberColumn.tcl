...
proc Construct_FiberColumn {eleID Node1 Node2 secID numSeg Initial_GI nInt Trans_tag Splice_status} {
...
if {$Splice_status==0} {
	...
	for {set i 1} {$i <= [expr $numSeg-1]} {incr i 1} {
		...
		set xGlobal [expr $X1+sin($PI*$i/$numSeg)*$Initial_GI*$L];
		set yGlobal [expr $Y1+$L/$numSeg*$i];		
