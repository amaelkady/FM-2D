    ...
    if Floor~=NStory+1 && Floor~=1
        for Axis=NBay+2:NBay+3
            ...
            if GFX==1
                ...
                fprintf(INP,'Spring_IMK  %7d %7d %7d $E $fy [expr (%.4f + %.4f)] %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f 0 $Composite 2 %d; ', SpringID,nodeID1,nodeID2,I_GC, Iy_MFcolumns(Story,1),SecData.d(idx), SecData.h_tw(idx), SecData.bf_tf(idx),SecData.ry(idx), H_GC,L_GC,Lb_GC,My_GC, Units);
            end
            if GFX==0
    ...
    if Floor==1
        for Axis=NBay+2:NBay+3
            ...
            if GFX==1
                ...
                fprintf(INP,'Spring_IMK  %7d %7d %7d $E $fy %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f 0 $Composite 2 %d; ', SpringID,nodeID1,nodeID2, Iy_MFcolumns(Story,1),SecData.d(idx), SecData.h_tw(idx), SecData.bf_tf(idx),SecData.ry(idx), H_GC,L_GC,Lb_GC,My_GC, Units);
            end
            if GFX==0
                fprintf(INP,'Spring_Zero %7d %7d %7d; ', SpringID, nodeID1,nodeID2);
            end
