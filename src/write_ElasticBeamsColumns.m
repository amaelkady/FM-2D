function [EL_ELEMENTS] = write_ElasticBeamsColumns (INP, NStory, NBay, FrameType, BraceLayout, ColElementOption, MF_COLUMNS, MF_BEAMS, Splice, initialGI, nIntegration, Units)
    ...
    if Splice(Story,1)==0
        for Axis=1:NBay+1
            ...
            if ColElementOption==1
                fprintf(INP,'element ModElasticBeam2d %d %d %d  %.4f $E [expr ($n+1)/$n*%.4f] $K11_2 $K33_2 $K44_2 $trans_selected; ',ElemID,nodeIDb,nodeIDt,SecData.Area(idx),SecData.Ix(idx));
        ...
        for Axis=1:NBay+1
            ...
            if ColElementOption==1
                fprintf(INP,'element ModElasticBeam2d %d %d %d %.4f $E [expr ($n+1)/$n*%.4f] $K33_1 $K11_1 $K44_1 $trans_selected;  ',ElemID02,nodeIDsplice72,nodeIDb,SecData.Area(idx1),SecData.Ix(idx1));
            elseif ColElementOption==2
                secID=secID+1;
        ...
        for Axis=1:NBay+1
            ...
            if ColElementOption==1
                fprintf(INP,'element ModElasticBeam2d %7d %7d %7d %.4f $E [expr ($n+1)/$n*%.4f] $K33_1 $K11_1 $K44_1 $trans_selected;  ',ElemID01,nodeIDt,nodeIDsplice71,SecData.Area(idx),SecData.Ix(idx));
            elseif ColElementOption==2
                secID=secID+1;
...
for Floor=NStory+1:-1:2
    if (FrameType==2 && BraceLayout==1 && mod(Floor,2)~=0) || FrameType==1
        for Bay=1:NBay
            ...
            fprintf(INP,'element ModElasticBeam2d %7d %7d %7d  %.4f $E [expr ($n+1)/$n*0.90*$Comp_I*%.4f] $K11_2 $K33_2 $K44_2 $trans_selected; ',ElemID,nodeIDr,nodeIDl,SecData.Area(idx),SecData.Ix(idx));
            EL_ELEMENTS(count,1)=ElemID;
            count=count+1;
        end
    else
        for Bay=1:NBay
            ...
            fprintf(INP,'element ModElasticBeam2d %7d %7d %7d  %.4f $E [expr ($n+1)/$n*0.90*$Comp_I*%.4f] $K11_2 $K33_2 $K44_2 $trans_selected; ',ElemID01,nodeIDr,nodeIDrC,SecData.Area(idx),SecData.Ix(idx));
            EL_ELEMENTS(count,1)=ElemID01;
            count=count+1;
            fprintf(INP,'element ModElasticBeam2d %7d %7d %7d  %.4f $E [expr ($n+1)/$n*0.90*$Comp_I*%.4f] $K11_2 $K33_2 $K44_2 $trans_selected; ',ElemID02,nodeIDl,nodeIDlC,SecData.Area(idx),SecData.Ix(idx));
            EL_ELEMENTS(count,1)=ElemID02;
            count=count+1;
        end
    end
