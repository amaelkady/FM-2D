function write_ElasticBeamsColumns(INP)

global MainDirectory
load(strcat(MainDirectory,'\temp_unpacked'), 'NStory', 'NBay',  'FrameType', 'BraceLayout', 'ColElementOption','BeamElementOption', 'MF_COLUMNS', 'MF_BEAMS',  'MF_SL', 'Splice', 'initialGI','nSegments', 'nIntegration', 'Material',  'SteelMatID', 'Units','Ry_column','Ry_beam');
        ...
        if Splice(Story,1)==0
            for Axis=1:NBay+1
                ...
                if ColElementOption==1
                    factor_I_mod = get_mod_I_with_shear ('Column', 0, Story, SecData, idx);
            ...
            for Axis=1:NBay+1
                ...
                if ColElementOption==1
                    factor_I_mod = get_mod_I_with_shear ('Column', 0, Story, SecData, idx1);
    ...
    for Floor=NStory+1:-1:2
        if (FrameType==2 && BraceLayout==1 && mod(Floor,2)~=0) || FrameType==1
            ...
            for Bay=1:NBay
                ...
                fprintf(INP,'element ModElasticBeam2d %8d %8d %8d  %.4f $E [expr ($n+1)/$n*0.90*$Comp_I*%.4f*%.4f] $K11_2 $K33_2 $K44_2 $trans_selected; ',ElemID01,nodeIDr,nodeIDrC,SecData.Area(idx),SecData.Ix(idx),factor_I_mod);
                ...
                if FrameType ==3 && BraceLayout ==2
                    ...
                    factor_I_mod = get_mod_I_with_shear ('Beam', Bay, 0, SecData, idx);
                    fprintf(INP,'element ModElasticBeam2d %8d %8d %8d  %.4f $E [expr ($n+1)/$n*0.90*$Comp_I*%.4f*%.4f] $K11_2 $K33_2 $K44_2 $trans_selected; ',ElemID02,nodeIDl,nodeIDlC,SecData.Area(idx),SecData.Ix(idx),factor_I_mod);
                    EL_ELEMENTS(count,1)=ElemID02;
                    count=count+1;
                end
            end
