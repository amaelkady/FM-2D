function write_BeamSpring_MRF_RC (INP, NStory, NBay, WBay, ModelELOption, MF_COLUMNS, MF_BEAMS, a_sl, Units)
...
if ModelELOption==2
    ...
    for Floor=NStory+1:-1:2
        ...
        for Axis=1:NBay+1
            ...
            if Axis~=1
                fprintf(INP,'Spring_IMK_RC %d %d %d $fc $Ec $fyR $Er %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %d 0.0 %d; ', SpringID_L, Node20, Node02, SecData.B, SecData.H, SecData.coverH, SecData.s, SecData.rho_Top, SecData.rho_Bot, SecData.rho_Int, SecData.rho_Shear, a_sl, Units);
            end
            if Axis~=NBay+1
                fprintf(INP,'Spring_IMK_RC %d %d %d $fc $Ec $fyR $Er %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %d 0.0 %d; ', SpringID_R, Node04, Node40, SecData.B, SecData.H, SecData.coverH, SecData.s, SecData.rho_Top, SecData.rho_Bot, SecData.rho_Int, SecData.rho_Shear, a_sl, Units);
            end
        end
        fprintf(INP,'\n');
    end
