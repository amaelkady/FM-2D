function [Pc]=write_ColumnSpring_RC (INP, NStory, NBay, HStory, ColElementOption, PM_Option, MF_COLUMNS, MF_BEAMS, fc, a_sl, Units)
    ...
    for Axis=1:NBay+1
        ...
        if Floor~=NStory+1 && Floor~=1           
            if ColElementOption==1
                fprintf(INP,'Spring_IMK_RC %d %d %d $fc $Ec $fyR $Er %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %d %.3f %d;\n', SpringID03, iNode03, jNode03, SecData.B, SecData.H, SecData.coverH, SecData.s, SecData.rho_Top, SecData.rho_Bot, SecData.rho_Int, SecData.rho_Shear, a_sl, PPc, Units);
                fprintf(INP,'Spring_IMK_RC %d %d %d $fc $Ec $fyR $Er %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %d %.3f %d;  ', SpringID01, iNode01, jNode01, SecData.B, SecData.H, SecData.coverH, SecData.s, SecData.rho_Top, SecData.rho_Bot, SecData.rho_Int, SecData.rho_Shear, a_sl, PPc, Units);
            else
        ...
        if Floor==1
            ...
            if ColElementOption==1
                fprintf(INP,'Spring_IMK_RC %d %d %d $fc $Ec $fyR $Er %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %d %.3f %d; ', SpringID01 ,iNode0, jNode03, SecData.B, SecData.H, SecData.coverH, SecData.s, SecData.rho_Top, SecData.rho_Bot, SecData.rho_Int, SecData.rho_Shear, a_sl, PPc, Units);
            else
